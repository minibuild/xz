		...
		uint32_t match_byte, uint32_t symbol)
{
	...
	if (!match_mode) {
		...
		do {
			uint32_t match_bit, subcoder_index, bit;
		...
		uint32_t position)
{
	...
	uint32_t rep_max_index, i;
	uint8_t current_byte, match_byte;
	uint32_t pos_state;
	uint32_t match_price, rep_match_price, normal_match_price;
		...
		const uint32_t nice_len, const uint32_t buf_avail_full)
{
	...
	for (rep_index = 0; rep_index < REPS; ++rep_index) {
		uint32_t len_test, len_test_temp, price, len_test_2, limit;

		const uint8_t *const buf_back = buf - reps[rep_index] - 1;
		if (not_equal_16(buf, buf_back))
			continue;

		len_test = lzma_memcmplen(buf, buf_back, 2, buf_avail);
	...
	if (new_len >= start_len) {
		uint32_t i, len_test, normal_match_price;
